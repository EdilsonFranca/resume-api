...
public class GeradorPDFM3 implements GeradorPDFStrategy {
    ...
    public ByteArrayOutputStream generatePdf(Resume resume) throws Exception {
        ...
        if (!resume.getExperiences().isEmpty()) {
            ...
            for (Experience e : resume.getExperiences()) {
                ...
                doc.add(jobWithDate(dateText, e.getPosition() + " - " + e.getNameCompany(), font));

                doc.add(new Paragraph(e.getDescription()).setFont(font).setFontSize(11).setMarginBottom(8));
            }
        }
        // ===== EDUCATION =====
    ...
    private static Table jobWithDate(String date, String title, PdfFont font) {
        Table table     = new Table(UnitValue.createPercentArray(new float[]{20, 80})).useAllAvailableWidth();
        Cell dateCell   = new Cell().add(new Paragraph(date).setFont(font).setFontSize(11)).setBorder(Border.NO_BORDER);
        Cell titleCell  = new Cell().add(new Paragraph(title).setFont(font).setFontSize(11)).setBorder(Border.NO_BORDER);
